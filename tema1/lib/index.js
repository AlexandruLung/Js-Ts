...
let arrayNumbers = [];
console.time();
fs_1.default.readFile("./ChangeBaseNumbers.csv", "utf8", (err, file) => {
    let lines = file.split("\n");
    for (let line = 1; line < lines.length - 1; line++) {
        arrayNumbers = lines[line].split(",");
        convertedArray.push(convertFromBaseToBase(parseInt(arrayNumbers[0]), parseInt(arrayNumbers[1]), parseInt(arrayNumbers[2])));
    }
